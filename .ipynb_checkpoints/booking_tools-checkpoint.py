...
class Booker:
    def __init__(self, location, from_, to_, adults, children, rooms, sort, holiday_length, airport_from=False,
                 airport_to=False, review_score=6, mealplan='', twin_beds='', stars='', distance=''):
        ...
        for map_ in range(len(order_map[self.sort])):
            for i in range(range_):
                ...
                query = {
                    ...
                    "nflt": f"ht_id=204;review_score={self.review_score}" + mealplan_str + twin_str + stars_str + distance_str
                }
    ...
    async def booking_search(self):
        ...
        async with aiohttp.ClientSession(headers=HEADERS, connector=connector) as session:
            ...
            tasks = [self.extract_hotels_from_page(html, date) for html, date in zip(html_pages, dates)]
        ...
        if (self.airport_from != False) & (self.airport_to != False):
